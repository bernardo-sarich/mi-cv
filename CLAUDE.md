...
- **`Infrastructure`** — implementa los puertos: `Persistence/` (EF Core contra Neon Postgres vía `CvDbContext`, provider `Npgsql.EntityFrameworkCore.PostgreSQL`, un repositorio por entidad) y `Email/SmtpContactNotifier.cs` (notificación por SMTP/Gmail vía MailKit). Las migraciones EF viven en `Infrastructure/Migrations/`. `EfCvRepository.GetCvAsync` dispara sus cuatro lecturas (profile+stats, experience, projects, skills) en paralelo vía `IDbContextFactory<CvDbContext>` + `Task.WhenAll` — cada `DbContext` no es thread-safe, así que cada lectura concurrente usa su propia instancia creada por la factory, nunca el contexto scoped inyectado.

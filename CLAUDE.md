...
- **`Infrastructure`** — implementa los puertos: `Persistence/` (EF Core contra Neon Postgres vía `CvDbContext`, provider `Npgsql.EntityFrameworkCore.PostgreSQL`) y `Email/SmtpContactNotifier.cs` (notificación por SMTP/Gmail vía MailKit). El read-side del CV es un repositorio por aggregate, no uno todo-en-uno: `EfProfileRepository`, `EfExperienceRepository`, `EfProjectRepository` y `EfSkillCategoryRepository` implementan cada uno su propio puerto (`IProfileRepository`, etc.) con acceso simple a su tabla, sin componer nada. Es `GetCvUseCase` (en `Application`) quien orquesta: dispara las cuatro lecturas en paralelo vía `Task.WhenAll` y arma el `CvContent` — cada implementación usa su propia instancia de `DbContext` (vía `IDbContextFactory<CvDbContext>`) porque no es thread-safe para uso concurrente. El write-side de contacto ya seguía este patrón: `EfContactRepository` (`ContactMessage`) y `EfContactAttemptStore` (`ContactAttempt`). Las migraciones EF viven en `Infrastructure/Migrations/`.

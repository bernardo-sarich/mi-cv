...
The Experience section SHALL render inside a `<section id="experience">` element, preceded by a `SectionLabel` and followed by an `h2` heading identifying the section.

#### Scenario: Section renders with label and heading
- **WHEN** the Experience section is rendered
- **THEN** a `<section>` element with id `experience` is present, containing a `SectionLabel` element followed by an `h2` heading
...
Each job entry SHALL be visually hidden (transparent, offset downward) until it enters the viewport, at which point it SHALL animate to fully visible with a fade-in and slide-up transition, using an IntersectionObserver to detect visibility.

#### Scenario: Job is hidden before entering the viewport
- **WHEN** a job entry has not yet scrolled into the viewport
- **THEN** it remains transparent and offset from its final position

#### Scenario: Job reveals on entering the viewport
- **WHEN** a job entry scrolls into the viewport for the first time

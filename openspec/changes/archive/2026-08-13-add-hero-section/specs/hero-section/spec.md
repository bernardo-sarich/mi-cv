...
The Hero section SHALL render two call-to-action buttons: a primary-styled "Descargar CV" button with no attached download behavior, and a secondary-styled "Ver proyectos →" button that smoothly scrolls the viewport to the `proyectos` section when activated.
...
- **THEN** the viewport smoothly scrolls until the section with id `proyectos` is in view, without a full page reload

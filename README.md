...
**Por qué**: quería demostrar manejo de EF Core migrations, un repositorio por aggregate (`EfProfileRepository`, `EfExperienceRepository`, `EfProjectRepository`, `EfSkillCategoryRepository` — cada uno accede solo a su tabla; es `GetCvUseCase`, en `Application`, quien arma la respuesta de `GET /api/cv` juntando los cuatro; el lado de contacto sigue el mismo patrón, con `EfContactRepository` y `EfContactAttemptStore` cada uno por su cuenta), y una herramienta de seed (`Seed/`) que puebla la base desde el mismo JSON que el cliente usa como mock — separando "la forma en la que edito mi contenido" (un JSON a mano) de "la forma en la que se sirve" (filas relacionales, versionadas con migraciones). El `Seed` es deliberadamente destructivo por locale en cada corrida (borra e inserta de nuevo); es una herramienta de desarrollo, no algo que corra en producción.
